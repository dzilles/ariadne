...
DOCKER_COMPOSE_CONTENT = """x-db-env: &db-env
...
ENV_CONTENT = """APP_RELEASE=stable
...
def setup_plane():
    ...
    plane_dir = os.path.join(root_dir, ".plane")
    env_file = os.path.join(plane_dir, ".env")
    compose_file = os.path.join(plane_dir, "docker-compose.yml")

    print("Setting up local Plane instance...")
    ...
    if not os.path.exists(plane_dir):
        print(f"Creating .plane directory at {plane_dir}...")
        os.makedirs(plane_dir)

    print("Writing embedded docker-compose.yml...")
    with open(compose_file, "w", encoding="utf-8") as f:
        f.write(DOCKER_COMPOSE_CONTENT)

    print("Writing default .env with Ariadne configuration...")
    with open(env_file, "w", encoding="utf-8") as f:
        f.write(ENV_CONTENT)

    print("Starting Plane services...")
    if run_command("docker compose up -d", cwd=plane_dir):
        print("Plane instance started successfully!")
        print("Access it at http://localhost:8090")
    else:
        print("Failed to start Plane instance.")
        sys.exit(1)
