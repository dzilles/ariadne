...
class OrchestratorAgent(BaseAgent):
    ...
    def delegate_to_agent(self, agent_name: str, ticket_id: str, task_description: str) -> str:
        ...
        Args:
            ...
            ticket_id: The Plane Sequence ID (number) of the ticket.
        ...
        try:
            ...
            if agent_name in status_map:
                ...
                self.pm_tools.update_ticket(ticket_number=ticket_id, state=target_status.value)
                self.pm_tools.add_comment(ticket_number=ticket_id, comment=f"Ariadne Orchestrator: Delegating task to {agent_name} agent. Moving ticket to {target_status.value}.")
...
2. **Knowledge Summary:** Summarize what you already know (e.g., from Plane comments or previous steps) so the agent doesn't have to re-discover it.
...
### Responsibilities:
1. **Monitor:** Track ticket status and progress in Plane.
