...
class LangGraphAdapter:
    ...
    def _handle_tool_end(self, event: dict) -> bool:
        ...
        if output is not None:
            # Try to get content attribute (for ToolMessage objects)
        ...
        # Check if output indicates an error (from error-handling wrapper)
        is_error = output_str.startswith("[Tool Error:")
        ...
        if is_error:
            self.response.tool_error(tool_name, output_str)
        else:
            # Record successful tool call (shown in verbose mode)
            self.response.tool_success(tool_name, output_str)
        return True
