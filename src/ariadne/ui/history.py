...
class HistoryError(Exception):
    """Base class for history-related errors."""
    pass
...
class HistoryFormatError(HistoryError):
    """Raised when the history file has invalid format/JSON."""
    pass
...
def load_history(agent: Any, agent_name: str) -> bool:
    ...
    Raises:
        HistoryNotFoundError: If the history file doesn't exist
        HistoryFormatError: If the history file contains invalid JSON
        HistoryError: For other loading failures (e.g., IO errors)
    """
    ...
    if not os.path.exists(filename):
        ...
        raise HistoryNotFoundError(f"No history file found for {agent_name} at {filename}")
    ...
    try:
        ...
        raise HistoryFormatError(f"Invalid JSON in history file {filename}: {e}") from e
    except Exception as e:
        logger.error(f"Failed to load history for {agent_name}: {e}")
        raise HistoryError(f"Failed to load history for {agent_name}: {e}") from e
